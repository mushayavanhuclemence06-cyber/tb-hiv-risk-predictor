...
def login_page():
    ...
    with st.form("login_form"):
        st.markdown("<label style='font-weight:700;'>Username</label>", unsafe_allow_html=True)
        username = st.text_input("", placeholder="Enter your username", label_visibility="collapsed")
        
        st.markdown("<label style='font-weight:700;'>Password</label>", unsafe_allow_html=True)
...
def predict_risk():
    ...
    if st.button("🔍 Predict Default Risk", type="primary", use_container_width=True):
        ...
        st.markdown(f'<div class="{css_class}" style="padding:1rem; border-radius:10px; margin:1rem 0;"><h3>{category}</h3><p><strong>Recommended Action:</strong> {action}</p></div>', unsafe_allow_html=True)
        ...
        st.markdown("---")
        st.caption(f"Predicted by: {users.get(st.session_state.username, {}).get('name', st.session_state.username)} | {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def patient_location_map():
    ...
    st.markdown("---")
    st.markdown("<h4>📍 Pinpoint Location - Add Patient</h4>", unsafe_allow_html=True)
    ...
    with st.form("pinpoint_form"):
        ...
        if st.form_submit_button("Add Patient at This Location", use_container_width=True):
            if p_name:
                patients = load_json(PATIENTS_FILE)
                pid = f"BUD-{len(patients)+1:04d}"
                patients[pid] = {
...
def main_app():
    ...
    with col1:
        ...
        <div class="header">
            ...
            <p>TB/HIV Treatment Default Risk Prediction System</p>
