...
def nutritional_assessment(patient_id, patient_name):
    ...
    with st.form(key=f"nutrition_form_{patient_id}"):
...
def send_sms_reminder(patient_name, phone, message_type):
    ...
    if message_type == "appointment":
        st.info("📱 **Message:** REMINDER: You have a clinic appointment tomorrow at Budiriro Clinic. Please bring your medication.")
        ...
        st.warning("⚠️ **Message:** URGENT: Our records show you missed your last appointment. Please call the clinic.")
    
    if st.button(f"📤 Send SMS", key=f"sms_btn_{patient_name}"):
