...
def send_sms(phone_number, patient_name, message_type, risk_level="low"):
    ...
    messages = {
        ...
        "mental_health": f"🧠 Mental Health Support: {patient_name}, speak to our counselor at Budiriro Clinic."
    }
...
def login_page():
    ...
    st.markdown("<h3 style='text-align:center; font-size:32px;'>TB/HIV Treatment Default Risk Prediction System</h3>", unsafe_allow_html=True)
    st.markdown("---")
...
def mental_health_screening(patient_id, patient_name):
    ...
    if total_score >= 15:
        st.error(f"Severe Depression - Score: {total_score}")
        save_alert(patient_id, patient_name, "mental_health", f"PHQ-9 score: {total_score}", "high")
    elif total_score >= 10:
        st.warning(f"Moderate Depression - Score: {total_score}")
        save_alert(patient_id, patient_name, "mental_health", f"PHQ-9 score: {total_score}", "medium")
    elif total_score >= 5:
        st.info(f"Mild Depression - Score: {total_score}")
    else:
        st.success(f"Minimal depression - Score: {total_score}")
...
def clinical_alerts_dashboard():
    ...
    col1.metric("Active Alerts", len(unresolved))
    col2.metric("High Priority", len(high_priority))
...
def chw_module():
    ...
    if high_risk_patients:
        st.warning(f"{len(high_risk_patients)} high-risk patients need home visits")
        ...
        st.success("No high-risk patients need home visits")
    ...
    if not patient_id:
        ...
        if st.button("Send SMS", use_container_width=True):
            if manual_name and manual_phone:
                send_sms(manual_phone, manual_name, message_type, "medium")
...
def predict_risk():
    ...
    def calculate_risk():
        ...
        if art_status == "Not on ART":
            ...
            alerts.append("Patient not on ART - start immediately")
        if weight < 50:
            points += 2
        ...
        if cd4 < 200:
            ...
            alerts.append("Advanced immunosuppression - expedite ART")
        if 18 <= age <= 24:
            points += 2
    ...
    if st.button("Predict Default Risk", type="primary", use_container_width=True):
        ...
        if alerts and patient_phone:
            if st.button("Send Urgent SMS"):
                send_sms(patient_phone, patient_name, "appointment", "high")
                st.success("Urgent SMS sent")
...
def view_patients():
    ...
    for pid, patient in patients.items():
        ...
        with st.expander(f"{pid} - {patient['name']} (Age: {patient['age']})"):
            tabs = st.tabs(["Info", "Nutrition", "Mental Health", "SMS"])
            with tabs[0]:
...
def patient_location_map():
    ...
    for patient in patients.values():
        if patient.get('location', {}).get('latitude'):
...
def main_app():
    ...
    with st.sidebar:
        ...
        menu = st.radio("MENU", [
            ...
            "Education", "Alerts", "CHW Module", "Send SMS"
        ])
