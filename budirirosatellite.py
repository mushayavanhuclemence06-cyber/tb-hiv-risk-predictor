...
def send_email(email_address, patient_name, message_type):
    ...
    email_templates = {
        "welcome": f"Welcome to Budiriro Clinic {patient_name}! We are here to support you.",
        "appointment": f"Reminder: {patient_name}, you have a clinic appointment tomorrow.",
        "medication": f"Medication Reminder: {patient_name}, time to take your TB medication.",
        "high_risk": f"URGENT: {patient_name}, please contact the clinic regarding your missed appointment."
    }
...
def edit_patient(patient_id, patient_data):
    ...
    with st.form(key=f"edit_form_{patient_id}"):
        ...
        with col1:
            ...
            new_email = st.text_input("Email Address", value=patient_data.get('email', ''), key=f"edit_email_{patient_id}")
        with col2:
        ...
        with col1:
            if st.form_submit_button("💾 Save Changes"):
                ...
                patients[patient_id]['email'] = new_email
...
def view_patients():
    ...
    # Create a list of patient IDs to iterate over
...
def main_app():
    ...
    with col1:
        ...
        <div style="background: linear-gradient(135deg, #1a5276, #2e86c1); padding: 2rem; border-radius: 15px;">
            <div style="font-size: 72px; font-weight: 900; color: #FFFFFF; text-align: center;">
