...
def nutritional_assessment(patient_id, patient_name):
    ...
    with st.form(key=f"nut_form_{patient_id}"):
        ...
        food_insecure = st.radio("In the past month, did you ever run out of food?", ["No", "Yes, sometimes", "Yes, often"])
            ...
            with st.expander(f"📍 {patient['name']} - {patient.get('location', {}).get('suburb', 'Unknown')}"):
                ...
                with col2:
                    if st.button(f"📅 Schedule Home Visit", key=f"schedule_{pid}"):
                        st.success(f"✅ Home visit scheduled")
...
def education_library():
    ...
    with tabs[0]:
        st.markdown("**TB Treatment Information**
- TB is curable with 6 months of medication
- Take medication daily
- Complete all doses")
    with tabs[1]:
        st.markdown("**HIV/ART Information**
- ART controls HIV
...
# ============================================
# MAIN APP - WITH LARGE WELCOME HEADER
# ============================================
def main_app():
    users_db = load_json(USERS_FILE)
    ...
    # LARGE WELCOME HEADER - Very Large and Bold
    st.markdown(f"""
