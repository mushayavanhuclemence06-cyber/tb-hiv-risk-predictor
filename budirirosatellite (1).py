...
if 'two_factor_verified' not in st.session_state:
...
def send_verification_code(phone):
    ...
    st.info(f"📱 Demo: Verification code for {phone} is: **{code}**")
    return True
...
def login_page():
    ...
    if not st.session_state.two_factor_verified:
        ...
        with st.form("2fa_form"):
            code = st.text_input("Enter 6-digit verification code", type="password")
            if st.form_submit_button("Verify"):
                if code == st.session_state.get('verification_code', ''):
                    st.session_state.logged_in = True
                    st.session_state.username = st.session_state.temp_username
                    st.session_state.user_name = load_json(USERS_FILE)[st.session_state.username]['name']
                    st.session_state.two_factor_verified = False
                    st.rerun()
                else:
...
# ============================================
# REGISTER PAGE WITH PHONE FOR 2FA
# ============================================
def register_page():
    ...
    with st.form("register_form"):
        ...
        phone = st.text_input("Phone Number for 2FA *", placeholder="e.g., 0771234567")
        ...
        if submitted:
            if not all([full_name, username, password, phone]):
                ...
                st.error("❌ Invalid phone number (e.g., 0771234567)")
            else:
                users = load_json(USERS_FILE)
                if username in users:
                    ...
                    users[username] = {
                        'name': full_name, 'password': hash_password(password), 'phone': phone,
...
def nutritional_assessment(patient_id, patient_name):
    st.markdown("<h3>🥗 Nutritional Assessment</h3>", unsafe_allow_html=True)
...
def mental_health_screening(patient_id, patient_name):
    st.markdown("<h3>🧠 Mental Health Screening</h3>", unsafe_allow_html=True)
    ...
    for i, q in enumerate(phq9_questions):
        score = st.radio(f"{i+1}. {q}", [0, 1, 2, 3], 
                         format_func=lambda x: ["Not at all", "Several days", "More than half days", "Nearly every days"][x],
    ...
    if total_phq9 >= 10:
        st.error(f"🔴 PHQ-9 Score: {total_phq9} - Moderate to Severe Depression detected")
        alert_msg = f"Patient {patient_name} has PHQ-9 score of {total_phq9} - needs mental health referral"
        save_alert(patient_id, patient_name, "mental_health", alert_msg)
    elif total_phq9 >= 5:
        st.warning(f"🟡 PHQ-9 Score: {total_phq9} - Mild Depression - Monitor")
    else:
...
def clinical_alerts_dashboard():
    ...
    st.warning(f"⚠️ {len(unresolved)} Active Alerts Requiring Attention")
        ...
        with st.expander(f"🔴 {alert['patient_name']} - {alert['type'].upper()} - {alert['date'][:10]}", expanded=True):
            ...
            with col1:
                if st.button(f"✅ Resolve Alert", key=f"resolve_{alert_id}"):
                    ...
                    st.rerun()
            with col2:
                st.caption(f"Alert ID: {alert_id}")

# ============================================
# PATIENT TIMELINE TRACKING
# ============================================
def patient_timeline(patient_id, patient_name):
    st.markdown("<h3>📅 Patient Treatment Timeline</h3>", unsafe_allow_html=True)
    ...
    for pred in patient_predictions:
        timeline_data.append({
            ...
            'Weight': nutrition.get(patient_id, {}).get('weight', 'N/A'),
            'PHQ-9': mental.get(patient_id, {}).get('phq9_score', 'N/A')
        })
...
def send_sms_reminder(patient_name, phone, message_type):
    ...
    st.write(f"**Message to {patient_name}:**")
    
    if message_type == "appointment":
        st.info("🔔 REMINDER: You have a clinic appointment tomorrow at Budiriro Clinic. Please bring your medication. Reply YES to confirm.")
    elif message_type == "medication":
        st.info("💊 REMINDER: Time to take your TB medication. Taking medication on time helps you get better faster.")
    elif message_type == "high_risk":
        st.warning("⚠️ URGENT: Our records show you missed your last appointment. Please call the clinic at 086-123-4567 to reschedule.")
...
def education_library():
    ...
    with tab1:
        ...
        **What to expect:**
        ...
        if st.button("📥 Download TB Information Sheet"):
            st.success("Download started - patient_education_tb.pdf")
...
def chw_module():
    ...
    for pid, patient in patients.items():
        # Check if patient has high risk predictions
    ...
    if high_risk_patients:
        st.warning(f"⚠️ {len(high_risk_patients)} high-risk patients need home visits")
            ...
            with st.expander(f"📍 {patient['name']} - {patient.get('location', {}).get('suburb', 'Unknown')}"):
                ...
                with col2:
                    ...
                    if st.button(f"📅 Schedule Home Visit", key=f"schedule_{pid}"):
                        ...
                        # Send SMS to CHW
                        st.info(f"📱 SMS sent to CHW: Visit {patient['name']} at {patient.get('location', {}).get('full_address', '')}")
    else:
...
# ============================================
# PREDICT RISK (Enhanced with Alerts)
# ============================================
def predict_risk():
...
def main_app():
    ...
    with col1:
        st.markdown(f"""
        <div style="background: linear-gradient(135deg, #1a5276, #2e86c1); padding: 1.5rem; border-radius: 10px; color: white; text-align: center;">
            <h1 style="color:white;">🏥 Budiriro Satellite Clinic</h1>
            <p style="font-size:18px;">TB/HIV Treatment Default Risk Prediction System</p>
            <p style="font-size:14px;">Welcome, {user_data.get('name', st.session_state.username)} | AUC = 0.706</p>
        </div>
        """, unsafe_allow_html=True)
    with col3:
